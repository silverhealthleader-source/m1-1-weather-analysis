...
## 덧붙여 — 기후변화, 그 다음은? (자율 탐구)

> 과제 요구사항에는 없는 내용입니다. 분석 도중 **2026년 8월 네팔 빙하 붕괴 사고**를 접하고
> "이 데이터가 가리키는 방향의 끝에 무엇이 있는가"가 궁금해 따로 찾아본 기록입니다.
> 본문과 달리 **직접 계산한 수치가 아니라 외부 기관 자료 인용**이므로 출처와 확인 시점을 모두 표기했습니다.

**한국의 미래 — 시나리오별 전망** (「한국 기후위기 평가보고서 2025」 환경부·기상청)

| 지표 | 기준<br>2000~2019 | 2021~2040 | 2081~2100<br>**저탄소** | 2081~2100<br>**고탄소** |
|---|---|---|---|---|
| 기온 상승 (산업화 이전 대비) | — | 1.6~1.8℃ | **2.3℃** | **7.0℃** |
| 폭염일수 | 8.8일 | 16.8~18.5일 | **24.2일** | **79.5일** |
| 열대야일수 | 3.2일 | 12.3~13.2일 | — | **68.4일** |

**실생활 실천 — 효과가 큰 순서**
식사(3.4 kgCO₂e/2끼) > 중거리 이동(8.0/왕복 80km) > 샤워(1.5/회) ≫ 휴대폰 충전(0.03/회)
→ "휴대폰 충전기 뽑기"와 "식사 한 끼"는 **약 57배** 차이입니다. 작은 실천이 무의미한 게 아니라,
**한정된 노력을 어디에 쓸지 정할 때 크기 순서를 알아야 한다**는 뜻입니다.

**우리 지역은? — 창원시 직접 조회** (기상청 기후변화 상황지도, 2026.9.23 조회)

| 창원시 | 2010년대 | 2050년대 | 2090년대 |
|---|---|---|---|
| 폭염일수 — 고탄소 | 13.1일 | **43.2일** | **100.8일** |
| 열대야일수 — 고탄소 | 8.9일 | **45.7일** | **89.4일** |

고탄소 시나리오에서 창원의 **겨울은 21세기 후반에 0일**이 되고, 여름은 118일 → **189일**로 늘어납니다.

**같은 창원시 안에서도 구별로 갈립니다.** 2010년대 폭염일수는 성산구 16.9일 ~ 진해구 9.3일로 **1.82배** 차이가 나고,
**의창구(내륙)와 마산합포구(해안)는 낮·밤 순위가 뒤집힙니다** — 낮 더위는 의창구가 1.53배 많지만 밤 더위는 마산합포구가 1.15배 많습니다.
→ **무더위쉼터(낮)와 야간 안부확인(밤)의 우선 지역이 다르다**는 뜻입니다.

![창원 5개 구의 차이](images/09_changwon_gu_gap.png)

다만 이 값은 **읍면동 평균의 격자 자료**라, 본 분석의 **창원 관측지점 실측값**과 직접 비교되지 않습니다.
2010년대에는 관측이 1.8배 많지만 2020년대 열대야는 반대로 전망이 1.24배 많아 **방향이 뒤집히므로**, 어느 한쪽이 높다고 말할 수 없습니다 — 그 대조를 부록 C ⑤에서 따로 다뤘습니다.

**지금 참여할 수 있는 제도** — [탄소중립포인트](https://cpoint.or.kr/) · [기후동행카드](https://www.seoul.go.kr/policy/view.do?id=1016) · [기후변화 상황지도](http://climate.go.kr/atlas)
→ 기후동행카드의 감축 효과는 **연간 약 3만 톤 CO₂**(서울연구원, 2025.1.26)로 발표됐으나, **설문 기반 추정치이고 승용차 전환율은 4%**라는 한계가 있습니다. 부록 C에 검증 내용을 실었습니다.

👉 네팔 사고 경위, 시나리오별 상세 전망, 실천 항목별 수치와 출처는 **[REPORT.md 부록 C](REPORT.md)** 에 있습니다.
...
| 적용한 시계열 분석 기법 | 3-2 |
| **트렌드 · 계절성 · 노이즈 — 같은 눈금으로 본 세 성분** | **3-3** (그림 11) |
| **이동평균 창 365일 · min_periods의 근거** | **3-2-1** (그림 12) |
| 시각화 10장 | 4장 |
...
```
 ...
 (374,340행)                                                nation_daily.csv (3,653행)
                                    ...
                                    ├──▶ step9_concept_visuals.py ──▶ images/11 · 12.png  (REPORT 3-3 · 3-2-1)
...
| step9 | `nation_daily.csv` | PNG 2장 | 3-3 · 3-2-1 |
...
├── images/           분석 결과 그래프 12장 (png) — 본문 01~07·10~12, 부록 C 08·09
...
├── step9_concept_visuals.py 개념 검증 그림 — 세 성분 · 이동평균 파라미터 (3-3 · 3-2-1)
├── requirements.txt
├── REPORT.md         분석 리포트
└── README.md         이 파일
```
