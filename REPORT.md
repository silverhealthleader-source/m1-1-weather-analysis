...
| 구분 | 폭염일수 | 열대야일수 |
|---|---|---|
| **본 분석** 창원 지점 관측 · 2016~2019 평균 | **23.2일** | **16.5일** |
| 상황지도 창원시 · 2010년대 | 13.1일 | 8.9일 |
| **본 분석** 창원 지점 관측 · 2020~2025 평균 | **22.3일** | **20.7일** |
| 상황지도 창원시 · 2020년대 (고탄소) | 21.7일 | 25.7일 |
| **본 분석** 창원 지점 관측 · 2024년 (최다 연도) | **45일** | **48일** |

2010년대를 보면 **같은 창원인데 폭염일수가 23.2일과 13.1일로 약 1.8배 차이 난다.** 원인은 전망이 틀려서가 아니라 **공간 평균의 범위가 다르기 때문**이다.

| 차이 요인 | 본 분석 | 상황지도 |
|---|---|---|
| 공간 단위 | 창원 **관측지점 1곳**(도심·저지대) | 창원시 **읍면동 값의 산술평균**(산지·농촌 포함) |
| 자료 성격 | ASOS 지점 관측 실측 | 1km 격자 기후자료(MK-PRISM)를 행정구역으로 변환 |
| 시간 단위 | 연 단위 (최다 연도 인용 가능) | 10년(연대) 평균 |

> **그래서 이 두 숫자는 서로를 반박하지 않는다.** 둘 다 맞고, 묻는 질문이 다를 뿐이다.
> 상황지도는 *"창원시 전역이 평균적으로 어떻게 되는가"*, 본 분석은 *"창원 도심 관측소에서 실제로 며칠이었는가"*를 답한다.
> **무더위쉼터를 몇 곳에 둘지**는 앞의 질문, **올여름 야간 안부확인을 며칠 해야 하는지**는 뒤의 질문이다.
> 이것이 본 리포트가 7-1에서 스스로에게 적용한 원칙 — **"숫자를 인용하기 전에 그 숫자가 어떤 범위를 평균한 것인지 먼저 확인한다"** — 의 실제 사례다.
...
| 기상청 「기후변화 상황지도」 — 창원시 직접 조회 (2026.9.23) | https://climate.go.kr/atlas |
