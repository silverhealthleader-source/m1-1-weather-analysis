...
| 구분 | 위치 |
|---|---|
| 코드 | `step1_load_clean.py` ~ `step8_rate_of_change.py` — 총 8개 (저장소 루트) |
| 리포트 | `REPORT.md` (이 문서) · `README.md` |
| 원본 데이터 | `data/raw/` — 기상청 ASOS 연도별 CSV 11개 |
| 그래프 | `images/` — PNG 10장 (본문 8장 + 부록 C 2장) |
...
| 요소 | 정의 | 이 데이터에서는 | 실제 값 |
|---|---|---|---|
| **추세 (Trend)** | 계절과 무관하게 **여러 해에 걸쳐 한 방향으로** 움직이는 성분 | 10년에 걸친 기온의 장기 상승분 | 365일 이동평균 끝점 차이 **+0.38℃**<br>선형회귀 기울기 **+1.22℃/10년** |
| **계절성 (Seasonality)** | **일정한 주기로 반복**되는 성분. 기온은 1년 주기가 지배적이다 | 여름·겨울의 규칙적인 오르내림 | 분해 결과 진폭 **25.96℃**<br>(−13.19 ~ +12.77, 매년 동일하게 반복) |
| **노이즈 (Noise) = 잔차 (Residual)** | 추세로도 계절성으로도 설명되지 않는 **나머지 전부**. 날씨의 우연한 변동 | 그날그날의 일교차, 특정 달의 이상 한파·폭염 | 표준편차 **σ = 1.01℃**<br>전체 변동의 **1.2%** |

> **한 문장으로** — 이 데이터의 변동 중 **98.8%는 추세와 계절성으로 설명되고, 나머지 1.2%가 노이즈**다.
> 세 성분을 실제로 분리한 결과와 분해 규칙은 **6-1**에, 노이즈(잔차)가 큰 달의 원인을 외부 자료로 대조한 결과는 **6-1의 표**에 있다.
> `statsmodels` 의 분해 함수는 이 성분을 `residual` 이라 부르므로, 본 리포트는 6장 이후 **'노이즈' 대신 '잔차'** 로 표기한다. 같은 것이다.

**왜 이 구분이 실제로 중요한가** — 세 성분의 크기가 완전히 다르기 때문이다.
계절성 진폭 **25.96℃** > 노이즈 σ **1.01℃** > 10년 추세 **+0.38℃** 순이다.
**찾으려는 신호(추세)가 셋 중 가장 작다.** 그래서 계절성을 걷어내지 않으면 추세는 보이지 않고, 이것이 365일 이동평균(3-2 ①)을 쓴 이유다.

---

## 4. 분석 결과 및 시각화

**시각화 목차** — 과제 기준은 필수 2개·권장 3개 이상이며, 본문에 총 **8장**을 포함했다. (부록 C에 자율 탐구 그림 2장이 별도로 있다. 파일 번호는 생성 순서이므로 목차 순서와 일치하지 않는다.)
...
│   ├── raw/          기상청 원본 CSV 11개
│   └── processed/    정제 결과 2개
├── images/           그래프 10장 (png) — 본문 01~07·10, 부록 C 08·09
├── step1_load_clean.py
├── step2_visualize.py
├── step3_bonus.py
...
| 목표 | 근거 위치 | 구두 답변 요지 |
|---|---|---|
| **④** **트렌드 / 계절성 / 노이즈** 개념을 설명할 수 있다 | **3-3** (정의·실제 값) · **6-1** (분해 결과) | 추세는 여러 해에 걸쳐 한 방향으로 가는 성분, 계절성은 1년 주기로 반복되는 성분, 노이즈(=잔차)는 둘로 설명되지 않는 나머지다. 이 데이터에서는 **계절성 진폭 25.96℃ > 노이즈 σ 1.01℃ > 10년 추세 +0.38℃** 순이다. **찾으려는 신호가 셋 중 가장 작다** — 그래서 계절성을 먼저 걷어내야 했다. |
| **⑤** **이동평균·변화율**을 왜/어떻게 적용했는지 설명할 수 있다 | **3-2** · **4-1** · **4-6** | **이동평균**: 창을 365일로 잡은 이유는 기온의 지배 주기가 1년이라 창을 정확히 1주기로 잡아야 계절 성분이 상쇄되기 때문이다. `min_periods=200`이면 첫 값이 상반기만 평균한 11.64℃가 되어 **+2.35℃라는 허수**가 나온다. 365로 고정해 **+0.38℃**로 바로잡았다.<br>**변화율**: 기온에는 **쓰지 않았다.** 섭씨는 0이 임의 기준점인 **간격척도**라, 같은 +0.70℃가 섭씨 기준 **+5.04%**, 켈빈 기준 **+0.245%**로 **약 21배** 달라진다. 대신 **비율척도인 폭염·열대야 일수에만** 변화율을 적용했다. |
...
| 목표 | 점검 전 상태 | 보완 내용 |
|---|---|---|
| **④** 트렌드·계절성·노이즈 개념 | 1장에 단어만 등장하고, 6-1에는 분해 **결과**만 있어 **개념 정의가 없었다.** '노이즈'와 '잔차'가 같은 것이라는 연결도 없었다 | **3-3 신설** — 세 요소의 정의·본 데이터의 실제 값·크기 순서를 표로 정리하고, 노이즈 = 잔차임을 명시 |
