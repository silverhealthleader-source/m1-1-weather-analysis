...
# ===== 그래프 1 : 추세 — 일평균기온 + 이동평균 =====
# [파라미터 선택 이유]
#   window=365 : 기온의 지배적 주기가 정확히 1년이므로, 창을 1주기로 잡아야
#                계절 성분이 상쇄되고 추세만 남는다. 300일이나 400일은 계절 성분이
#                완전히 상쇄되지 않아 추세선에 잔물결이 남는다.
#   min_periods=365 : 창이 다 차야만 값을 낸다. 200 등으로 낮추면 첫 값이
#                2016년 상반기(추운 계절)만의 평균이 되어 추세가 과장된다.
#                (실제로 min_periods=200일 때 +2.35℃, 365일 때 +0.38℃로 6배 차이)
#   window=30  : 월 단위 체감과 맞춘 보조선. 계절 주기를 드러내는 용도이며
#                추세 판단에는 사용하지 않는다. min_periods=15는 월의 절반.
...
# [임계값 선택 이유] 기상청 폭염주의보(일최고 33℃)·열대야(일최저 25℃) 공식 기준을
#   그대로 사용한다. 임의 기준을 쓰지 않아야 기상청 공식 통계와 직접 비교할 수 있다.
#   단점: 32.9℃ 같은 '바로 아래' 날은 집계에서 빠지므로, 기온이 전반적으로 오르면
#        일수가 계단식으로 급증할 수 있다(REPORT 3-2의 해석 유의점 참조).
